...
PRINCIPLES_TEXT = """
## 细节核查要点(逐项核查; 核查结论必须以自然语言融入【具体评价】对应维度的连贯段落,
评语中不得出现"审计条款""核查项""条款A/B"等程序化标签或编号)
1. 数值一致性: 摘要/正文/结论中同一指标的数值必须一致(如占比、金额、面积、高度、产量)。
   "从a增长至b"类表述的方向词必须与数字大小关系相符(升配升、降配降)。
2. 图表引用一致性: 每张图/表必须有图注/表注(如"图3-1 xxx"), 且正文中必须有对应引用
   ("如图3-1所示"/"见表4-2")。引用了不存在的编号、或有图无注、有注无引用, 均为缺陷。
3. 数量词一致性: "三大问题/四个模块/五大维度"类数量表述在摘要、正文、结论中必须统一,
   且与实际展开的小节数量相符。
4. 题目-内容匹配: 研究对象名称、地域、主体在题目/摘要/正文/结论中必须一致;
   研究对象的脱敏表述("XX公司""某高校""某村""A公司""B集团"等)是盲审常规形态, 视为有效研究对象,
   本身不构成任何缺陷, 不得据此扣分或判不合格; 真缺陷是错位(题目对象与正文对象不一致)
   或全文混用真实名称与脱敏名造成指代不清。
5. 摘要-结论呼应: 结论必须逐一回应摘要提出的研究问题, 不得引入摘要未提的新结论主体。
6. 表述规范: 同一对象全文称谓统一(全称/简称不混用造成歧义)。
"""
